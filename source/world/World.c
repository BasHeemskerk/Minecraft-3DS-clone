...
void World_Init(World* world, WorkQueue* workqueue, int worldSize, int worldHeight, int chunkSize) {

	int randomSeed = 0;

	int WORLD_SIZE = worldSize;
	int CHUNCK_SIZE = chunkSize;
	int WORLD_HEIGHT = worldHeight;
	...
	strcpy(world->name, "New World");

	world->workqueue = workqueue;

	world->genSettings.seed = randomSeed;
