...
void WorldSelect_ScanWorlds() {
	...
	DIR* directory = opendir("sdmc:/MinecraftHB/saves");
	...
	while ((entry = readdir(directory))) {
		sprintf(buffer, "sdmc:/MinecraftHB/saves/%s/level.mp", entry->d_name);
...
static WorldGenType worldGenType = WorldGen_Smea;

static char* worldGenTypesStr[] = {"Default", "Flat"};
...
void WorldSelect_Render() {
	...
	//SpriteBatch_BindGuiTexture(GuiTexture_TopMenuBackground);
...
bool WorldSelect_Update(char* out_worldpath, char* out_name, WorldGenType* worldType, bool* newWorld) {
	...
	if (confirmed_deletion) {
		...
		char buffer[512];
		sprintf(buffer, "sdmc:/MinecraftHB/saves/%s", worlds.data[selectedWorld].path);
		delete_folder(buffer);
