...
#define CLEAR_COLOR_WHITE 0xffffffff
...
void Renderer_Init(World* world_, Player* player_, WorkQueue* queue, GameState* gamestate_) {
	...
	Texture_Load(&logoTex, "romfs:/textures/gui/title/logoNew.png");
	//Texture_Load(&topTex, "romfs:/textures/gui/title/background/menuBackgroundTop.png");
...
void Renderer_Render() {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (*gamestate == GameState_SplashScreen){
			C3D_RenderTargetClear(renderTargets[i], C3D_CLEAR_ALL, CLEAR_COLOR_WHITE, 0);
			renderSplash();
		}
		else if (*gamestate == GameState_SelectWorld){
			C3D_RenderTargetClear(renderTargets[i], C3D_CLEAR_ALL, CLEAR_COLOR_BLACK, 0);

			C3D_Mtx projection;
			Mtx_PerspStereoTilt(&projection, C3D_AngleFromDegrees(0.f), ((400.f) / (240.f)), 0.22f, 4.f * CHUNK_SIZE,
					    !i ? -iod : iod, 3.f, false);
			...
			//SpriteBatch_PushText(1, 110, 2, INT16_MAX, true, INT_MAX, NULL, CRAFTUS_VERSION_STR);
			renderVersion();
			renderSplashText();
			renderPanorama();
			...
			SpriteBatch_PushQuad(30, 0, 5, 144, 36, 0, 0, 256, 64);
	...
	if (*gamestate == GameState_SplashScreen){
		C3D_RenderTargetClear(lowerScreen, C3D_CLEAR_ALL, CLEAR_COLOR_WHITE, 0);
	}
	else if (*gamestate == GameState_SelectWorld){
		C3D_RenderTargetClear(lowerScreen, C3D_CLEAR_ALL, CLEAR_COLOR_BLACK, 0);
		WorldSelect_Render();
